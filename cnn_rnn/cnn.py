...
# 对numpy数组进行element wise操作,shape不变
...
class ConvLayer(object):
	...
	def __init__(self, input_width, input_height, channel_number, filter_width, filter_height, filter_number,
		...
		for i in range(filter_number):
			self.filters.append(Filter(filter_width, filter_height,
			                           self.channel_number))  # 参数数量：(channel_num*filter_width*filter_height+1)*filter_number
	...
	def forward(self, input_array):
		...
		for f in range(self.filter_number):  # Feature Map的深度(个数)
			filter = self.filters[f]
			conv(self.padded_input_array, filter.get_weights(), self.output_array[f], self.stride, filter.get_bias())
		element_wise_op(self.output_array, self.activator.forward)  # 对于卷积结果矩阵中output_array中的每个元素进行relu激活计算
	...
	def bp_sensitivity_map(self, sensitivity_array, activator):
		...
		activator: 上一层的激活函数
		'''
		# 处理卷积步长，对原始sensitivity map进行维度扩展：shape=(3,3):（input_width-filter_width+2 * self.zero_padding+1）*（input_height-filter_height+2 * self.zero_padding+1）
	...
	def bp_gradient(self, sensitivity_array):
		...
		for f in range(self.filter_number):
			...
			# 计算偏置项的梯度就是sensitivity map所有误差项之和。
			filter.bias_grad = expanded_array[f].sum()

	def expand_sensitivity_map(self, sensitivity_array):
		'''
		是将步长为S的sensitivity map『还原』为步长为1的sensitivity map
		'''
	...
	def calculate_output_size(input_size, filter_size, zero_padding, stride):
		'''
		图像宽度W1=5，filter宽度F=3，Zero PaddingP=0，步幅S=2
		则Feature Map宽度是2=（W1-filter+2P）/S+1
		'''
